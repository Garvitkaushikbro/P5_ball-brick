...
function brick_contact() {
  for (r = 0; r < 4; r++) {
    for (c = 0; c < 4; c++) {
      ...
      if (
        ((ball_dy > 0 &&
          ballBottomSide >= brickTopSide &&
          ballBottomSide <= brickBottomSide) ||
          (ball_dy < 0 &&
            ballTopSide <= brickBottomSide &&
            ballTopSide >= brickTopSide)) &&
        ballRightSide >= brickLeftSide &&
        ballLeftSide <= brickRightSide
      ) {
        if (bricks[c][r].hidden == 0) {
          ball_dy = -ball_dy;
          bricks[c][r].hidden = 1;
